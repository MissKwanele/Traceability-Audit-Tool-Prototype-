...
This tool will visualize traceability, identify orphaned requirements, and generate a PDF audit report.
""")
...
if uploaded_file:
    ...
    st.write(f"- Orphaned items: **{len(orphans)}**")
    if orphans:
        st.write(orphans)
    ...
    pdf.section_body(f"Total requirements: {G.number_of_nodes()}")
    pdf.section_body(f"Total trace links: {G.number_of_edges()}")
    pdf.section_body(f"Orphaned items: {len(orphans)}")
    if orphans:
